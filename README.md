...
8. Optional: pass `authorized_verifiers` (comma-separated Jira account emails) to require that `required_status` was actually set by one of those people, not just anyone on the team. This is a way to enforce "only the PM or repo owner can mark something Dev Done" when your Jira plan doesn't support workflow transition restrictions natively (that's a Standard-plan-and-above feature — Free plan has no per-project workflow editor). The check walks the issue's changelog to find who performed the most recent transition into `required_status` and fails if that person isn't in the allowlist. Leave empty (the default) to skip this check entirely.
